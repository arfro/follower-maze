...
2. Sketch out the big picture and add/improve models (here: add)
...
### Choices
- using algebraic data types (sum for `Config`, product for `EventError`/`Event`) to model data
...
I use three 3rd party libraries, two of them in tests and one for config (I really hope using this one can be justified - if not, I obviously could have just use an object)
- TypeSafe config library that makes it nice and easy to work off `application.conf`
- ScalaTest library for unit tests
- Mockito for mocking dependencies on socket related tests

### Dead letter queue
I decided to take a very basic and simple approach in the dead letter queue - it's simply a mutable queue data structure.
Even though it is significantly slower than Vector (which is on the other hand the best choice for concurrent code though) it was important to keep the FIFO functionality to avoid situations such as sending unfollow message before follow.
...
It's durable and can be monitored, there could be alerts setup that notify the team when there is an unusually high amount of dead letter queue entries - this could signify application is misbehaving.

I would store a reason for why each of the messages is being in dead letter queue (I already took the first step which is to model message related error case classes, see `EventError`). I would replay the messages the are not delievered because the user was offline and perhaps purge the incorrect format ones. After some time when I know the average numbers of incorrect format messages (e.g. per day) I would set the alert on it. A large peak in those could indicate unhealthy behaviour from some clients which should be investigated. 

### Testing
Even though adding tests for part 1 was not in scope I decided to add a little bit for the new code I introduced. I like to have any convertes/exporters thoroughly tested (especially if they are an addition to the codebase). It's an area that can cause a lot of problems should not edge cases scenarios not be properly covered. 
You will notice one ignored test - as I didn't want to expand the time budgeted for the task any further I decided to leave as is. Please see `Improvements and notes` for explanation.

### Improvements and notes
Things I would not ship to prod as is:
- `MessageConverter.createEventInner` - this one would require more work and a proper error handler. As mentioned above, I added a test case that I marked as ignored - sending a non-digit String would result in an Exception
